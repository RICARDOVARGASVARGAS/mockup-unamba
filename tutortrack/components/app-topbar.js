 ...
 *   user-ciclo     — opcional; solo aplica a Estudiante (no todos los
 *                    roles tienen un Ciclo en el modelo de datos — ver
 *                    docs/MODELO-DATOS.md). Si no se pasa, no se muestra.
 *
    ...
    connectedCallback() {
      ...
      this.innerHTML = `
        <header class="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border bg-bg px-4 sm:px-6">
          ...
          <h1 class="font-heading text-lg font-semibold text-text truncate">${title}</h1>
          ...
          <div class="ml-auto flex items-center gap-1">
            ...
            <details class="group relative">
              <summary class="relative flex h-10 w-10 cursor-pointer list-none items-center justify-center rounded-md text-text-muted transition hover:bg-surface-2 hover:text-text [&::-webkit-details-marker]:hidden">
                ${icon("bell")}
                <span class="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-danger"></span>
              </summary>
              <div class="absolute right-0 top-full z-20 mt-2 w-72 rounded-lg border border-border bg-surface p-2 shadow-lg">
                <p class="px-2 py-1 text-xs font-semibold uppercase tracking-wide text-text-muted">Notificaciones</p>
                <a href="#" class="mt-1 block rounded-md px-2 py-2 text-sm transition hover:bg-surface-2">
            ...
            <details class="group relative ml-1">
              <summary class="flex cursor-pointer list-none items-center gap-2 rounded-md py-1.5 pl-2 pr-1 transition hover:bg-surface-2 [&::-webkit-details-marker]:hidden">
                <span class="flex h-8 w-8 items-center justify-center rounded-full bg-primary text-sm font-semibold text-onPrimary">${userInitials}</span>
